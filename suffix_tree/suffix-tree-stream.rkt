...
;; Extracts the first branch of a suffix tree (ST)
...
(define (get-ch-branch st ch)
  (if (stream-empty? st)
      #false  ; If the stream is empty, return #false indicating the branch was not found
      (let ((branch (stream-first st)))
        (if (char=? ch (car (car branch)))  ; Check if the first character of the label matches `ch`
            branch  ; If so, return the current branch
            (get-ch-branch (stream-rest st) ch)))))  ; Otherwise, recurse with the rest of the stream
