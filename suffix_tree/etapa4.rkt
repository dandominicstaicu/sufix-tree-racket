  ...
  (define (search-for-substring node accumulated)
    (if (collection-empty? node) ; Check if there are no more branches to explore.
        ...
        (let* ((branch (collection-first node)) ; Assuming collection-first returns the first branch correctly.
          ...
          (cond
            ((and (>= (length new-accumulated) len) ; If accumulated labels match or exceed desired length,
                  (not (collection-empty? subtree))) ; and there's more of the tree to explore,
             (take new-accumulated len)) ; return the accumulated labels up to the specified length.
            (else
